...
public class MidgameBaseLogic extends ABaseLogic {
    private final int NR_OF_CONVERTERS = 1; //Amount of clerics with convertOpponents behavior
    private UnitType nextUnit = null;
    private UnitBehavior nextUnitBehavior = null;
    private double fighterPioneerRatio = 0.24;
    private Random random = new Random();

    public MidgameBaseLogic(MoveFactory moveFactory, CustomContext context, BaseBehavior baseBehavior) {
        super(moveFactory, context, baseBehavior);
    }

    //At the start we want to grow as fast as possible. For this we create workers at first and expand territory if we need to increase unit cap
    @Override
