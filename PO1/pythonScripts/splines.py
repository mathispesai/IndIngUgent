...
def main():
    x = sp.symbols('x')
    g = rechte([1,3],[3,7]); g
    print('De vergelijking van de rechte door [1,3] en [3,7]')
    print('y =',g(x))
    f = parabool([1,3],[3,7],1); f
    print('De vergelijking van de parabool door [1,3] en [3,7] met m = 1')
    print('y =',f(x))
    #D = dict(a=97,b=98,c=99); D
    #D['a']
    #D[0]
    X = [1,3, 4,6, 8,16,20]
    Y = [3,7,-1,9,11,19,15]
    gi = koorden(X,Y)
    print('Koorden door de punten met X =',X,'en Y =',Y)
    print([g(x) for g in gi])
    m = sp.symbols('m')
    fi = splines(X,Y,m)
    print('Splines door de punten met X =',X,'en Y =',Y)
    print([f(x) for f in fi])
    print('Kwadratische oppervlakte in functie van de parameter m')
    print('en de optimale parameter')
    print(optimaal(X,Y))
    figuur(X,Y)
